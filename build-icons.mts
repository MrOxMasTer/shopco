...
import config from './svgo.config.mts';

// Creating ways and checking them
const cwd = process.cwd();
const inputDir = path.join(cwd, 'src', 'shared', 'assets', 'svg');
const inputDirRelative = path.relative(cwd, inputDir);
const typeDir = path.join(cwd);
...
async function generateIconFiles() {
  //SPRITE and TYPES file generation
...
// The function of creating icon names
...
async function generateSvgSprite({
  ...
  const symbols = await Promise.all(
    files.map(async (file) => {
      const FullPathFile = path.join(inputDir, file);

      const input = await fsExtra.readFile(FullPathFile, 'utf8');

      const result = optimize(input, {
        path: FullPathFile, // recommended
        ...config,
      });
